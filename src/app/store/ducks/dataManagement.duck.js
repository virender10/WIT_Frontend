...
import { getFormFieldList, getFormStepsList, saveFormData, addField } from "../../services/dataManagementService";
...
export const actions = {
  ...
  addFieldSuccess: data => ({ type: actionTypes.AddFieldSuccess, payload: { data } }),
  ...
  saveFormDataSuccess: steps => ({ type: actionTypes.SaveFormDataSuccess, payload: { steps } }),
  getFormSteps: userid => ({ type: actionTypes.GetFormSteps, payload: { userid } }),
  getFormStepSuccess: (steps) => ({ type: actionTypes.GetFormStepsSuccess, payload: { steps } }),
};
...
export function* saga() {
  yield takeLatest(actionTypes.AddField, function* addFieldSaga({ payload }) {
    ...
    // }
    yield addField(data);
    yield put(actions.addFieldSuccess(data));
  });
  yield takeLatest(actionTypes.GetFormField, function* createUserSaga({ payload }) {
    const { step } = payload;
  ...
  yield takeLatest(actionTypes.SaveFormData, function* getUserListSaga({ payload }) {
    // const { id } = payload;
